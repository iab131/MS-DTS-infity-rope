...
def assemble_memory_context(retrieved_kv, local_key, local_value, frame_tokens, current_frames, mode):
    ...
    return (
        torch.cat([
            local_key[:, :sink_end],
            retrieved_key,
            local_key[:, retained_recent_start:current_start],
            local_key[:, current_start:],
        ], dim=1),
        torch.cat([
            local_value[:, :sink_end],
            retrieved_value,
            local_value[:, retained_recent_start:current_start],
            local_value[:, current_start:],
        ], dim=1),
    )
